...
async function displayPest(req, res) {
    ...
    try {
        ...
        const pest = await ngsiLD.readEntity(String(req.params.id), { options: 'keyValues' }, ngsiLD.setHeaders(req.session.access_token, LinkHeader));
        return res.render('pest', { title: pest.name, pest });
    }
    catch (error) {
        const err = error;
        const errorDetail = err.cause;
        log(errorDetail);
