...
async function displayWeather(req, res) {
    ...
    try {
        ...
        const weather = await ngsiLD.readEntity(String(req.params.id), { options: 'keyValues' }, ngsiLD.setHeaders(req.session.access_token, LinkHeader));
        return res.render('weather', { title: weather.name, weather });
    }
    catch (error) {
        const err = error;
        const errorDetail = err.cause || error;
        log(errorDetail);
