...
import { withSessionLoad, type Session } from '../../lib/session';
...
export function registerContextDiscoveryResources(server: FastMCP<Session>): void {
    server.addResource({
        uri: 'ngsi://types',
        ...
        load: withSessionLoad(() => liveJson(() => listTypes(true)))
    });
    ...
    server.addResource({
        ...
        load: withSessionLoad(() => liveJson(() => listAttributes(true)))
    });
}
