...
export function registerGeoQuery(server: FastMCP): void {
    server.addTool({
        ...
        parameters: z.object({
            ...
            coordinates: z
                .string()
                .describe('GeoJSON coordinates as a JSON string, e.g. "[-3.12,40.41]" or "[[[-3.1,40.4],[-3.2,40.4],[-3.2,40.5],[-3.1,40.4]]]".'),
            geoproperty: z
                .string()
                .optional()
                .describe('Which GeoProperty to test against (default "location").'),
        ...
        execute: async ({ type, georel, geometry, coordinates, geoproperty, pick, limit }) => {
            try {
